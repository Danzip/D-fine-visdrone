#!/bin/bash
# Watchdog for p2_640 experiment.
# First run: tunes from checkpoint (resets optimizer).
# Subsequent restarts: resumes from last.pth.
# Usage: bash experiments/p2_640/watchdog.sh

set -euo pipefail
cd "$(dirname "$0")/../.."

CONFIG="experiments/p2_640/config.yml"
TUNING_CKPT="output/dfine_hgnetv2_s_visdrone_nwd/best_stg1_dfine_s_visdrone_nwd_sqrt.pth"
RESUME_CKPT="output/p2_640/last.pth"
LOG="output/p2_640/watchdog.log"
RESTART_DELAY=15

mkdir -p output/p2_640
