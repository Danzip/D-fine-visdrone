...
def run_yolo(raw_bytes: bytes, threshold: float, suffix: str = ".jpg"):
    # Write to a temp file so YOLO reads it exactly like sota_compare.py did
    with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
        tmp.write(raw_bytes)
        tmp_path = tmp.name
    image = Image.open(io.BytesIO(raw_bytes))
    orig_w, orig_h = image.size
    results = yolo_model(tmp_path, conf=threshold, iou=0.45, verbose=False)[0]
...
async def detect(
    ...
    if model == "yolov8":
        detections, orig_w, orig_h = run_yolo(data, threshold, suffix)
    else:
