...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, weight_dict, use_focal_loss=False, alpha=0.25, gamma=2.0, nwd_constant=0.5):
        super().__init__()
    ...
    def forward(self, outputs: Dict[str, torch.Tensor], targets, return_topk=False):
        ...
        # NWD cost: treat each box as Gaussian N(cx,cy, w/2, h/2); NWD = exp(-W/C) in [0,1]
        if self.cost_nwd > 0:
            # out_bbox, tgt_bbox in cxcywh normalized format
            p = out_bbox.unsqueeze(1)   # [Q, 1, 4]
            g = tgt_bbox.unsqueeze(0)   # [1, T, 4]
            # Wasserstein: sqrt((cx1-cx2)^2 + (cy1-cy2)^2 + (w1/2-w2/2)^2 + (h1/2-h2/2)^2)
            diff = p - g  # [Q, T, 4]
            diff[..., 2:] = diff[..., 2:] / 2.0
            wasserstein = diff.pow(2).sum(-1).sqrt()  # [Q, T]
            nwd = torch.exp(-wasserstein / self.nwd_constant)
            cost_nwd = 1.0 - nwd
        else:
            cost_nwd = torch.zeros_like(cost_giou)
