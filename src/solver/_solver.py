...
class BaseSolver(object):
    ...
    def _setup(self):
        ...
        # Optionally freeze backbone + encoder, train only the decoder.
        # Use when adapting to a new resolution: backbone and encoder are largely
        # resolution-invariant (CNN local features + CCFF convolutions). The decoder
        # (enc_output, enc_score_head, cross_attn sampling_offsets, FDR heads) is what
        # determines WHERE to look and HOW to refine boxes — it needs to re-adapt.
        if self.cfg.yaml_cfg.get("freeze_except_decoder", False):
            frozen, unfrozen = 0, 0
            for name, param in self.model.named_parameters():
                if name.startswith("decoder.") and param.is_floating_point():
                    param.requires_grad = True
                    unfrozen += param.numel()
                else:
                    param.requires_grad = False
                    frozen += param.numel()
            print(f"[freeze_except_decoder] Frozen {frozen:,} params (backbone+encoder), training {unfrozen:,} params (decoder)")
    ...
    def train(self):
        ...
        # Give the collate fn access to the dataset for AR-aware mosaic sampling
        if hasattr(self.train_dataloader.collate_fn, "set_dataset"):
            self.train_dataloader.collate_fn.set_dataset(
        ...
        # Optional secondary val dataloader (e.g. val_dataloader_43 for 4:3 eval)
        _val_43 = getattr(self.cfg, "val_dataloader_43", None)
        self.val_dataloader_43 = (
            dist_utils.warp_loader(_val_43, shuffle=_val_43.shuffle) if _val_43 is not None else None
        )
    ...
    def load_state_dict(self, state):
        ...
        for k, v in self.__dict__.items():
            if hasattr(v, "load_state_dict") and k in state:
                v = dist_utils.de_parallel(v)
                if k == "model":
                    # Use shape-matched load so resolution-dependent buffers
                    # (decoder.anchors, decoder.valid_mask) don't cause a crash
                    # when eval_spatial_size differs from the checkpoint's size.
                    stat, infos = self._matched_state(v.state_dict(), state[k])
                    v.load_state_dict(stat, strict=False)
                    if infos["unmatched"]:
                        print(f"Load {k}.state_dict (skipped shape-mismatched: {infos['unmatched']})")
                    else:
                        print(f"Load {k}.state_dict")
                elif k == "ema":
                    # Apply the same shape-matched load for the EMA module copy
                    ema_module = dist_utils.de_parallel(v).module
                    ema_state = state[k]["module"]
                    stat, infos = self._matched_state(ema_module.state_dict(), ema_state)
                    ema_module.load_state_dict(stat, strict=False)
                    if "updates" in state[k]:
                        dist_utils.de_parallel(v).updates = state[k]["updates"]
                    if infos["unmatched"]:
                        print(f"Load {k}.state_dict (skipped shape-mismatched: {infos['unmatched']})")
                    else:
                        print(f"Load {k}.state_dict")
