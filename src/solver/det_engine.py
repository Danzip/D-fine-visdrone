    ...
    **kwargs,
    ...
    for i, (samples, targets) in enumerate(
        ...
        try:
            ...
            prefix = "[OOM]" if is_oom else "[ERROR]"
            error_msg = (
                f"{prefix} epoch={epoch} batch={i} type={type(e).__name__}: {e}\n"
                f"{traceback.format_exc()}"
            )
            print(error_msg)
            if not is_oom and output_dir is not None:
                with open(os.path.join(str(output_dir), "error_log.txt"), "a") as _ef:
                    _ef.write(error_msg + "\n")
            optimizer.zero_grad()
            if scaler is not None:
                try:
                    ...
                    pass  # exception before backward — scaler has no inf checks, nothing to update
        ...
        # W&B image visualization once per epoch at end-of-epoch (cloud only — no TB to save disk)
        if (
            ...
            try:
                ...
                print(f"[WandbViz/W&B] Epoch image logging failed at epoch {epoch}: {e}")
    ...
    if use_wandb:
        wandb_log = {"epoch": epoch, "train/loss": np.mean(losses)}
        for j, pg in enumerate(optimizer.param_groups):
            wandb_log[f"train/lr_pg{j}"] = pg["lr"]
        for k, meter in metric_logger.meters.items():
            if k != "lr":
                wandb_log[f"train/{k}"] = meter.global_avg
        wandb.log(wandb_log)
...
def evaluate(
    ...
    **kwargs,
    ...
    # VisDrone scenes can have 100-800 objects; default maxDets=100 caps recall at 12% for
    # dense images. 500 lets COCO eval see all predictions in the typical VisDrone range.
    for _eval_obj in coco_evaluator.coco_eval.values():
        _eval_obj.params.maxDets = [1, 10, 500]
