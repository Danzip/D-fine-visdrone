...
class DetSolver(BaseSolver):
    def fit(self):
        ...
        if self.use_wandb:
            ...
            # Epoch-level metrics use "epoch" as their x-axis, not the global step counter.
            # Without this, wandb plots them against the step (~55k) instead of epoch (~15).
            wandb.define_metric("train/epoch_*", step_metric="epoch")
            wandb.define_metric("metrics/*", step_metric="epoch")

        # Per-class visualizer — logs 5 GT-vs-pred images per class each epoch.
        # Initialised when W&B or TensorBoard is enabled AND val dataset has an ann_file.
        wandb_viz = None
        if self.use_wandb or self.writer:
            try:
