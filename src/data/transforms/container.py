...
class Compose(T.Compose):
    ...
    def default_forward(self, *inputs: Any) -> Any:
        ...
        prev_len = len(sample) if isinstance(sample, (tuple, list)) else 1
        for transform in self.transforms:
            sample = transform(sample)
            # Guard: a transform must not grow the tuple beyond 2 unless it is
            # explicitly 3-tuple-aware. The dataset legitimately passes (img, tgt, self)
            # at entry; that is covered by prev_len on the first iteration.
            cur_len = len(sample) if isinstance(sample, (tuple, list)) else 1
            if cur_len > prev_len and cur_len > 2:
                name = type(transform).__name__
                known_3tuple = {"CopyPasteSmallObjects", "Mosaic"}
                if name not in known_3tuple:
                    raise ValueError(
                        f"Transform '{name}' grew the sample tuple from {prev_len} to "
                        f"{cur_len} elements. Only CopyPasteSmallObjects/Mosaic may emit "
                        f"3-tuples. Check the transform's forward() return value."
                    )
            prev_len = cur_len
        return sample
